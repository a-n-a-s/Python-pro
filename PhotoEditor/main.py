...
def drawOnImage(canvas):
    ...
    style.configure('TButton', font=('calibri', 23, 'bold'),
                    borderwidth='1')
    # ----------------------------------------
    ...
    redButton = ttk.Button(drawFrame, bg="red", width=2,
                           command=lambda: colourChosen(drawWindow, canvas, "red"))
    redButton.grid(row=0, column=0)
    blueButton = ttk.Button(drawFrame, bg="blue", width=2,
...
def colourPop(canvas):
    ...
    canvas.data.drawOn = False
    messagebox.showinfo(
        title="Colour Pop", message="Click on a part of the image which you want in colour", parent=canvas.data.mainWindow)
    if canvas.data.cropPopToHappen == False:
        canvas.data.mainWindow.bind(
            "<ButtonPress-1>", lambda event: getPixel(event, canvas))
        ...
        if canvas.data.colourPopToHappen == True and \
           canvas.data.cropPopToHappen == False and canvas.data.image != None:
            ...
            pixelr, pixelg, pixelb = \
                canvas.data.image.getpixel(
                    (canvas.data.pixelx, canvas.data.pixely))
    ...
    canvas.data.image =\
        canvas.data.image.crop(
            (int(round((canvas.data.startCropX-canvas.data.imageTopX)*canvas.data.imageScale)),
             int(round((canvas.data.startCropY-canvas.data.imageTopY)
                       * canvas.data.imageScale)),
             int(round((canvas.data.endCropX-canvas.data.imageTopX)
                       * canvas.data.imageScale)),
...
def desktopBk(canvas):
    if canvas.data.image != None:
        ...
        ctypes.windll.user32.SystemParametersInfoA(
            SPI_SETDESKWALLPAPER, 0, str(newLocation), 0)
...
def buttonsInit(root, canvas):
    ...
    cropButton = ttk.Button(toolKitFrame, text="Crop",
                            command=lambda: crop(canvas))
    cropButton.grid(row=0, column=0)
    rotateButton = ttk.Button(toolKitFrame, text="Rotate",
                              command=lambda: rotate(canvas))
    rotateButton.grid(row=1, column=0)
    brightnessButton = ttk.Button(toolKitFrame, text="Brightness",
                                  command=lambda: brightness(canvas))
    brightnessButton.grid(row=2, column=0)
    histogramButton = ttk.Button(toolKitFrame, text="Histogram",
                                 command=lambda: histogram(canvas))
    histogramButton.grid(row=3, column=0)
    colourPopButton = ttk.Button(toolKitFrame, text="Colour Pop",
                                 command=lambda: colourPop(canvas))
    colourPopButton.grid(row=4, column=0)
    mirrorButton = ttk.Button(toolKitFrame, text="Mirror",
                              command=lambda: mirror(canvas))
    mirrorButton.grid(row=5, column=0)
    flipButton = ttk.Button(toolKitFrame, text="Flip",
                            command=lambda: flip(canvas))
    flipButton.grid(row=6, column=0)
    transposeButton = ttk.Button(toolKitFrame, text="Transpose",
                                 command=lambda: transpose(canvas))
    transposeButton.grid(row=7, column=0)
    drawButton = ttk.Button(toolKitFrame, text="Draw",
                            command=lambda: drawOnImage(canvas))
    drawButton.grid(row=8, column=0)
    resetButton = ttk.Button(toolKitFrame, text="Reset",
